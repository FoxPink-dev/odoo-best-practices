...
Warningthis means you cannot define a field and a method with the same
...
TransientModel for temporary data, stored in the database but
automatically vacuumed every so often
AbstractModel for abstract super classes meant to be shared by
...
browse(), search(), or field accesses. Records have no
explicit representation: a record is represented as a recordset of one
record.
To create a class that should not be instantiated,
the _register attribute may be set to False.
...
Automatically defaults to True for Model and
TransientModel, False for AbstractModel.


TipTo create a model without any table, inherit
from AbstractModel.
...
Warningif multiple fields with the same name are defined in the
...
Alongside a parent_path field, sets up an indexed storage
...
_transient_max_count or _transient_max_hours conditions
(if any) are reached.
...
string (str) – the label of the field seen by users; if not
...
index (str) – whether the field is indexed in database, and the kind of index.
Note: this has no effect on non-stored and virtual fields.
The possible values are:

"btree" or True: standard index, good for many2one

"btree_not_null": BTREE index without NULL values (useful when mostvalues are NULL, or when NULL is never searched for)
...
company_dependent (bool) – whether the field value is dependent of the current company;
...
group_operator (str) – aggregate function used by read_group()
...
group_expand (str) – function used to expand read_group results when grouping on
the current field.
@api.model
...
See alsoAdvanced Fields/Compute fields


precompute (bool) – whether the field should be computed before record insertion
...
WarningPrecomputation only happens when no explicit value and no default
...
compute_sudo (bool) – whether the field should be recomputed as superuser
to bypass access rights (by default True for stored fields, False
for non stored fields)
recursive (bool) – whether the field has recursive dependencies (the field
...
See alsoAdvanced fields/Related fields
...
ExampleTo round a quantity with the precision of the unit of measure:
...
Parameters
attachment (bool) – whether the field should be stored as ir_attachment
...
sanitize_overridable (bool) – whether the sanitation can be bypassed by
the users part of the base.group_sanitize_override group (default: False)
sanitize_tags (bool) – whether to sanitize tags
(only a white list of attributes is accepted, default: True)
sanitize_attributes (bool) – whether to sanitize attributes
(only a white list of attributes is accepted, default: True)
sanitize_style (bool) – whether to sanitize style attributes (default: False)
strip_style (bool) – whether to strip style attributes
...
NoteIf no max_width/max_height is specified (or is set to 0) and verify_resolution is False,
...
Parameters
currency_field (str) – name of the Many2one field
...
selection (list(tuple(str,str)) or callable or str) – specifies the possible values for this field.
It is given as either a list of pairs (value, label), or a model
method, or a method name.
selection_add (list(tuple(str,str))) – provides an extension of the selection in the case
...
ExampleTo parse date/datetimes coming from external sources:
...
WarningStrings representing dates and datetimes can be compared
...
NoteThis function may be used to compute default values.
...
NoteThis method may be used to compute default values.


Parameters

record – recordset from which the timezone will be obtained.
timestamp (datetime) – optional datetime value to use instead of
...
WarningIf a datetime object is given as value,
...
NoteThis method is not meant for use as a default initializer,
because datetime fields are automatically converted upon
display on client side. For default values, now()
...
to provide domain)
context (dict) – an optional context to use on the client side when
handling that field
ondelete (str) – what to do when the referred record is deleted;
possible values are: 'set null', 'restrict', 'cascade'
auto_join (bool) – whether JOINs are generated upon search through that
field (default: False)
delegate (bool) – set it to True to make fields of the target model
accessible from the current model (corresponds to _inherits)
check_company (bool) – Mark the field to be verified in
_check_company(). Add a default company
...
comodel_name (str) – name of the target model
inverse_name (str) – name of the inverse Many2one field in
comodel_name
domain – an optional domain to set on candidate values on the
client side (domain or a python expression that will be evaluated
to provide domain)
...
relation (str) – optional name of the table that stores the relation in
the database
column1 (str) – optional name of the column referring to “these” records
in the table relation
column2 (str) – optional name of the column referring to “those” records
in the table relation
...
with self.
In case of a Many2many relation, removing the
...
In case of a One2many relation, the given record
...
The field value is stored as a string following the pattern
...
Contrary to Reference fields, the model has to be specified
in a Char field, whose name has to be specified in the
model_field attribute for the current Many2oneReference field.
...
WarningWhile it is possible to use the same compute method for multiple
...
TipYou can specify precise field dependencies if you don’t want
...
WarningYou cannot chain Many2many or One2many fields in related fields dependencies.
...
Warning_log_access must be enabled on
TransientModel.
...
Sets active to False on a recordset, by calling
toggle_active() on its currently active records.




Model.action_unarchive()[source]¶
Sets active to True on a recordset, by calling
toggle_active() on its currently inactive records.
...
WarningContrary to what the name implies, it is currently possible for
...
NoteWhen accessing non-relational fields on a recordset of potentially multiple
records, use mapped():
...
WarningTrying to read a field on multiple records will raise an error for non relational
fields.
...
See alsoThe methods search_fetch() and
...
Warning@constrains only supports simple field names, dotted names
...
DangerSince @onchange returns a recordset of pseudo-records,
...
WarningIt is not possible for a one2many or many2many field to modify
...
downgrade – a function downgrade(self, value, *args, **kwargs)
to convert the record-style value to a traditional-style output
upgrade – a function upgrade(self, value, *args, **kwargs)
...
Parameters
at_uninstall (bool) – Whether the decorated method should be called if
...
DangerThe parameter at_uninstall should only be set to True if the
...
WarningNo sanity checks applied in sudo mode!
...
WarningWhen using an unauthorized company for current user,
...
NoteThe returned recordset has the same prefetch object as self.
...
WarningUsing sudo could cause data access to cross the
...
WarningExecuting raw SQL bypasses the ORM and, by consequent, Odoo security rules.
...
join(args: Iterable) → odoo.tools.sql.SQL[source]¶
Join SQL objects or parameters with self as a separator.




classmethod identifier(name: str, subname: str | None = None) → odoo.tools.sql.SQL[source]¶
...
Example# make sure that 'partner_id' is up-to-date in database
...
Example# make sure 'state' is up-to-date in database
...
vals_list, and if necessary those from default_get().

Parameters
vals_list (Union[list[dict], dict]) – values for the model’s fields, as a list of dictionaries:
...
UserError – if a loop would be created in a hierarchy of objects a result of the operation
...
Parameters
default (dict) – dictionary of field values to override in the
...
NoteUnrequested defaults won’t be considered, there is no need to return a
...
Create a new record by calling create() with only one value
provided: the display name of the new record.
The new record will be initialized with any default values
applicable to this model, or provided through the context. The usual
behavior of create() applies.
...
For numeric fields (Integer,
Float) the value should be of the
corresponding type
For Boolean, the value should be a
bool
...
The expected value of a One2many or
Many2many relational field is a list of
Command that manipulate the relation the
...
clear(), and
set().
For Date and ~odoo.fields.Datetime,
the value should be either a date(time), or a string.


WarningIf a string is provided for Date(time) fields,
...
Returns the number of records in the current model matching the
provided domain.
...
to the cache.  This method is like a combination of methods search()
and fetch(), but it performs both tasks with a minimal number of
SQL queries.
...
This method is equivalent to calling search() with a search
...
args (list) – optional search domain (see search() for
...
load (str) – loading mode, currently the only option is to set to
...
domain (list) – A search domain. Use an empty
list to match all records.
...
aggregates (list) – list of aggregates specification.
...
order (str) – optional order by specification, for
...
offset (int) – optional number of groups to skip
limit (int) – optional max number of groups to return
orderby (str) – optional order by specification, for
overriding the natural sort ordering of the
groups, see also search()
(supported only for many2one fields currently)
lazy (bool) – if true, the results are only grouped by the first groupby and the
...
__range: (date/datetime only) dictionary with field_name:granularity as keysmapping to a dictionary with keys: “from” (inclusive) and “to” (exclusive)
...
operator (str)an operator used to compare the field_name with the value. Valid
operators are:
...
=likematches field_name against the value pattern. An underscore
_ in the pattern stands for (matches) any single character; a
percent sign % matches any string of zero or more characters.

likematches field_name against the %value% pattern. Similar to
=like but wraps value with ‘%’ before matching

not likedoesn’t match against the %value% pattern

ilikecase insensitive like

not ilikecase insensitive not like

=ilikecase insensitive =like

inis equal to any of the items from value, value should be a
list of items

not inis unequal to all of the items from value

child_ofis a child (descendant) of a value record (value can be either
one item or a list of items).
Takes the semantics of the model into account (i.e following the
relationship field named by
_parent_name).

parent_ofis a parent (ascendant) of a value record (value can be either
one item or a list of items).
Takes the semantics of the model into account (i.e following the
relationship field named by
_parent_name).

anymatches if any record in the relationship traversal through
field_name (Many2one,
One2many, or Many2many)
satisfies the provided domain value.

not anymatches if no record in the relationship traversal through
field_name (Many2one,
One2many, or Many2many)
satisfies the provided domain value.
...
valuevariable type, must be comparable (through operator) to the named
field.
...
NoteMostly to negate combinations of criteria
...
ExampleTo search for partners named ABC, with a phone or mobile number containing 7620:
[('name', '=', 'ABC'),
...
NoteSince V13, multi-relational field access is supported and works like a mapped call:
...
key (callable or str or None) – either a function of one argument that returns a
...
Noteunlike itertools.groupby(), does not care about input
ordering, however the tradeoff is that it can not be lazy


Parameters
key (callable | str) – either a callable from a Model to a (hashable)
...
NoteIt will also yield the various automatic fields unless they’ve been disabled
...
Warningwhen using delegation inheritance, methods are not inherited,
only fields
...
NoteIf you consider introducing new exceptions,
...
button_text (str) – text to put on the button that will trigger
the redirection.
additional_context (dict) – parameter passed to action_id.
...
ExampleWhen you try to log with a wrong password.
...
ExampleWhen you try to read a record that you are not allowed to.
...
ExampleWhen you try to read a value in a flushed cache.
...
ExampleWhen you try to write on a deleted record.
...
ExampleWhen you try to create a new user with a login which already exist in the db.
